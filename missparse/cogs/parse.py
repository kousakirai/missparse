...
class NoteData:
    ...
    def get_first_image(self):
        ...
        if images[0]["issensitive"]:
            return self._mozaic(images[0]["bytes"])
        else:
...
class MissParser(commands.Cog):
    ...
    async def on_message(self, message):
        for url in self.urls:
            if url + "/notes/" in message.content:
                note = await NoteData.create(message.content[message.content.rfind('/') + 1:])
                embed = discord.Embed(
                    title="リンク展開",
                ...
                view.add_item(OriginalNote(message.content[message.content.rfind('/') + 1:]))
                if note.get_images():
